...
class TestPlanningPublishing(TestCommonPlanning):
    ...
    def setUpClass(cls):
        ...
        cls.shift = cls.env['planning_old.slot'].create(values)
    ...
    def test_planning_publication(self):
        ...
        self.assertTrue(self.shift.is_published, 'planning_old is is_published when we call its action_send')

        shift_mails = set(Mails.search([])) ^ set(before_mails)
        self.assertEqual(len(shift_mails), 1, 'only one mail is created when publishing planning_old')
