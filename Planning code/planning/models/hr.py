...
class Employee(models.Model):
    ...
    planning_role_id = fields.Many2one('planning_old.role', string="Default Planning Role", groups='hr.group_hr_user')
    ...
    def _planning_get_url(self, planning):
        ...
        for employee in self:
            if employee.user_id and employee.user_id.has_group('planning_old.group_planning_user'):
                result[employee.id] = '/web?#action=planning_old.planning_action_open_shift'
            else:
                result[employee.id] = '/planning_old/%s/%s' % (planning.access_token, employee.employee_token)
        return result
