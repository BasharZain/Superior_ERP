...
class PlanningRecurrency(models.Model):
    _name = 'planning_old.recurrency'
    _description = "Planning Recurrence"

    slot_ids = fields.One2many('planning_old.slot', 'recurrency_id', string="Related planning_old entries")
    ...
    def _repeat_slot(self, stop_datetime=False):
        for recurrency in self:
            slot = self.env['planning_old.slot'].search([('recurrency_id', '=', recurrency.id)], limit=1, order='start_datetime DESC')
            ...
            if slot:
                ...
                self.env['planning_old.slot'].create(slot_values_list)
    ...
    def _delete_slot(self, start_datetime):
        slots = self.env['planning_old.slot'].search([('recurrency_id', 'in', self.ids), ('start_datetime', '>=', start_datetime), ('is_published', '=', False)])
        slots.unlink()
