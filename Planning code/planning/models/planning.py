...
class Planning(models.Model):
    ...
    role_id = fields.Many2one('planning_old.role', string="Role")
    ...
    is_published = fields.Boolean("Is the shift sent", default=False, readonly=True, help="If checked, this means the planning_old entry has been sent to the employee. Modifying the planning_old entry will mark it as not sent.")
    ...
    template_autocomplete_ids = fields.Many2many('planning_old.slot.template', store=False, compute='_compute_template_autocomplete_ids')
    template_id = fields.Many2one('planning_old.slot.template', string='Planning Templates', store=False)

    # Recurring (`repeat_` fields are none stored, only used for UI purpose)
    recurrency_id = fields.Many2one('planning_old.recurrency', readonly=True, index=True, ondelete="set null", copy=False)
    ...
    repeat_until = fields.Datetime("Repeat Until", help="If set, the recurrence stop at that date. Otherwise, the recurrence is applied indefinitely.")
    ...
    def _compute_allocation_type(self):
        for slot in self:
            if slot.start_datetime and slot.end_datetime and (slot.end_datetime - slot.start_datetime).total_seconds() / 3600.0 < 24:
                slot.allocation_type = 'planning_old'
            else:
    ...
    def _compute_allocated_hours(self):
        for slot in self:
            if slot.start_datetime and slot.end_datetime:
                ...
                if slot.allocation_type == 'planning_old' and slot.start_datetime and slot.end_datetime:
    ...
    def _compute_template_autocomplete_ids(self):
        ...
        self.template_autocomplete_ids = self.env['planning_old.slot.template'].search(domain, order='start_time', limit=10)
    ...
    def _inverse_repeat(self):
        for slot in self:
            if slot.repeat and not slot.recurrency_id.id:  # create the recurrence
                ...
                recurrence = self.env['planning_old.recurrency'].create(recurrency_values)
    ...
    def _inverse_template_creation(self):
        ...
        existing_templates = self.env['planning_old.slot.template'].read_group([], ['role_id', 'start_time', 'duration'], ['role_id', 'start_time', 'duration'], limit=None, lazy=False)
        ...
        self.env['planning_old.slot.template'].create([x for x in values_list if x not in existing_values])
    ...
    def _onchange_template_autocomplete_ids(self):
        templates = self.env['planning_old.slot.template'].search([], order='start_time', limit=10)
        if templates:
    ...
    def write(self, values):
        # detach planning_old entry from recurrency
        ...
        if any(key in ('repeat', 'repeat_type', 'repeat_until', 'repeat_interval') for key in values):
            ...
            for slot in self:
                if slot.recurrency_id and values.get('repeat') is None:
                    ...
                    slot.recurrency_id._delete_slot(slot.end_datetime)
                    # Create recurrence B
                    recurrence = slot.env['planning_old.recurrency'].create(recurrency_values)
                    slot.recurrency_id = recurrence
                    slot.recurrency_id._repeat_slot()
        return result
    ...
    def action_self_assign(self):
        """ Allow planning_old user to self assign open shift. """
        self.ensure_one()
    ...
    def action_self_unassign(self):
        """ Allow planning_old user to self unassign from a shift, if the feature is activated """
        self.ensure_one()
        # The following condition will check the read access on planning_old.slot, and that user must at least 'read' the
    ...
    def action_send(self):
        group_planning_user = self.env.ref('planning_old.group_planning_user')
        template = self.env.ref('planning_old.email_template_slot_single')
        # update context to build a link for view in the slot
        view_context = dict(self._context)
        view_context.update({
            'menu_id': str(self.env.ref('planning_old.planning_menu_root').id),
            'action_id': str(self.env.ref('planning_old.planning_action_open_shift').id),
            'dbname': self.env.cr.dbname,
            'render_link': self.employee_id.user_id and self.employee_id.user_id in group_planning_user.users,
            'unavailable_path': '/planning_old/myshifts/',
        })
    ...
    def _read_group_employee_id(self, employees, domain, order):
        if self._context.get('planning_expand_employee'):
            return self.env['planning_old.slot'].search([('create_date', '>', datetime.now() - timedelta(days=30))]).mapped('employee_id')
        return employees


class PlanningRole(models.Model):
    _name = 'planning_old.role'
...
class PlanningPlanning(models.Model):
    _name = 'planning_old.planning_old'
    ...
    slot_ids = fields.Many2many('planning_old.slot', "Shifts", compute='_compute_slot_ids')
    ...
    def _compute_slot_ids(self):
        ...
        for planning in self:
            ...
            planning.slot_ids = self.env['planning_old.slot'].search(domain)
    ...
    def send_planning(self, message=None):
        ...
        sent_slots = self.env['planning_old.slot']
        for planning in self:
            # prepare planning_old urls, recipient employees, ...
            slots = planning.slot_ids
            slots_open = slots.filtered(lambda slot: not slot.employee_id)

            # extract planning_old URLs
            employees = slots.mapped('employee_id')
            employee_url_map = employees._planning_get_url(planning)

            # send planning_old email template with custom domain per employee
            template = self.env.ref('planning_old.email_template_planning_planning', raise_if_not_found=False)
            template_context = {
