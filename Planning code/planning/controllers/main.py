...
class ShiftController(http.Controller):
    ...
    @http.route(['/planning_old/<string:planning_token>/<string:employee_token>'], type='http', auth="public", website=True)
    def planning(self, planning_token, employee_token, message=False, **kwargs):
        ...
        planning_sudo = request.env['planning_old.planning_old'].sudo().search([('access_token', '=', planning_token)], limit=1)
        ...
        planning_slots = request.env['planning_old.slot'].sudo().search(domain)
        ...
        return request.render('planning_old.period_report_template', {
    ...
    @http.route('/planning_old/<string:token_planning>/<string:token_employee>/assign/<int:slot_id>', type="http", auth="public", methods=['post'], website=True)
    def planning_self_assign(self, token_planning, token_employee, slot_id, **kwargs):
        slot_sudo = request.env['planning_old.slot'].sudo().browse(slot_id)
        ...
        planning_sudo = request.env['planning_old.planning_old'].sudo().search([('access_token', '=', token_planning)], limit=1)
        if not planning_sudo or slot_sudo.id not in planning_sudo.slot_ids._ids:
            return request.not_found()

        slot_sudo.write({'employee_id': employee_sudo.id})
        return redirect('/planning_old/%s/%s?message=%s' % (token_planning, token_employee, 'assign'))

    @http.route('/planning_old/<string:token_planning>/<string:token_employee>/unassign/<int:shift_id>', type="http", auth="public", methods=['post'], website=True)
    def planning_self_unassign(self, token_planning, token_employee, shift_id, **kwargs):
        slot_sudo = request.env['planning_old.slot'].sudo().search([('id', '=', shift_id)], limit=1)
        ...
        return redirect('/planning_old/%s/%s?message=%s' % (token_planning, token_employee, 'unassign'))
