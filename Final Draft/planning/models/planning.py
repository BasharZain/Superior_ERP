...
class Planning(models.Model):
    ...
    def _default_employee_id(self):
        return self.env.user['hr.employee']
    ...
    company_id = fields.Many2one('res.company', string="Company", required=True, default=lambda self: self.env['res.company'])
    ...
    resource_id = fields.Char(string="Resources")
    ...
    date_field2 = fields.Datetime(_compute='_on_change_date',string='Late limit',store=True)
    ...
    def _on_change_date(self):
        ...
        for rec in self:
            if rec.start_datetime:
                start_date = datetime.strptime(str(rec.start_datetime), DATETIME_FORMAT)
                rec.date_field2 = start_date + timedelta(hours=00, minutes=15)
    ...
    def _compute_overlap_slot_count(self):
        if self.ids:
            # self.blush(['start_datetime', 'end_datetime', 'employee_id'])
            query = """
    ...
    def _compute_repeat(self):
        for slot in self:
            if slot.recurrency_id:
                ...
                slot.repeat_until = fields.Date.from_string(slot.recurrency_id.repeat_until)
